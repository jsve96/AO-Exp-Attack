           ...
           # delta = torch.einsum('cik,ckj ->cij',u,v)
            ...
            for BATCH in batch(self.data,batch_size):
                ...
                for i,frame in enumerate(BATCH):
                    ...
                    input = input/255.0
                    #print("####",i)
                    #adv_e = (input + eps*delta.permute(2,0,1)/delta_norm).clamp(0,1)
                    ...
                    loss += -1*loss_bg+ 1*loss_fg #+ 0.0001*confl
                #print(loss)
                #loss.backward()
            ...
            print("Attack Rate:",self.Attack_rate)
            print('Box Ratio', self.BoxRatio[-1])
            #print(delta)
            #self.Nuc_NORM_RUN.append(np.float64(nuc_nor(delta.detach().cpu())))
...
def main(args):
    ...
    def filter_perturbation(P,eps):
        P = np.clip(P,0,1)/np.clip(P,0,1).max()
        P[P<eps]=0
        return P.mean(axis=2)

    plt.imshow(filter_perturbation(Result,0.000001),cmap='binary')
    plt.savefig('Test_PGD.png')



    Metrics = {'dataset':args.dataset,'rank':args.rank,'eps':args.eps,'nit':args.nit,'IOU':np.float64(loraPGD.iou_loss.detach().cpu().numpy()),'IOU_run':loraPGD.IOU_RUN,'nuc_norm_run':loraPGD.Nuc_NORM_RUN}
...
if __name__ == "__main__":
    ...
    Data = ['PETS09']
    instance = ['View_00'+ str(i) for i in range(1,8)]#['View_001']
    #Save = ['/home/jacob/Repos/python/VideoAttacks/experiments/PETS09'+"/"+instance[0]+"/LoRaPGD"]
    Path_Data = ["/home/jacob/datasets"]
    Nit = [100]
    rank = [0.1,0.5,1.0]#[0.1,0.5,1.0]
    #eps = [0.5,1.0,3.0]
    eps=[10.0]
    ...
    for dataset,path_dataset,nit,eps,rank,instance in product(Data,Path_Data,Nit,eps,rank,instance):
        args = argparse.Namespace(
            ...
            eps=eps,
            rank=rank,
            save='/home/jacob/Repos/python/VideoAttacks/experiments/PETS09'+"/"+instance+"/LoRaPGD",
            instance = instance
        )
        print(args.dataset)
        main(args) 
